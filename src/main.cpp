...
static const int64 nLookbackInterval = 6; // 6 blocks lookback for difficulty adjustment
static const int64 nLookbackTimespan = nLookbackInterval * nTargetSpacing; // 15 minutes

static const int64 LimUp = nLookbackTimespan * 100 / 106; // 6% up
static const int64 LimDown = nLookbackTimespan * 106 / 100; // 6% down
...
bool CBlockHeader::CheckProofOfWork(int nHeight) const
{
    if (nHeight >= GetAuxPowStartBlock())
    {
        ...
        {
            // Check proof of work matches claimed amount
			uint256 powhash = GetPoWHash(nHeight);
			if (powhash == 0)
				return error("CheckProofOfWork() : Out of memory - 1");
...
unsigned char pchMessageStart[4] = { 0xf5, 0x03, 0xa9, 0x51 };
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        printf("mempool.countZCSpend = %lu\n", mempool.countZCSpend);
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZoinMiner, pwallet));
}
